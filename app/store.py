...
class ChatStore:
    ...
    def ping(self) -> bool:
        ...
        try:
            ...
            # Nuốt MỌI exception: /readyz dùng hàm này, một exception thoát ra
            # sẽ biến readiness probe thành lỗi 500 thay vì 503.
            return False
    ...
    def add_turn(self, client_id: str, role: str, content: str) -> None:
        ...
        # -N đến -1 = N phần tử CUỐI = mới nhất. (0, N-1) giữ nhầm phần cũ nhất.
        self.client.ltrim(key, -HISTORY_MAX_MESSAGES, -1)
        # Hội thoại cũ tự hết hạn, không thì Redis đầy dần đến khi sập

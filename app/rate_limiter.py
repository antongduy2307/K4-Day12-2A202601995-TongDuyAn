...
class TokenBucket:
    ...
    def available(self, client_id: str, now: float | None = None) -> float:
        ...
        if not state:
            # Client mới → xô đầy. Khởi tạo rỗng thì request đầu tiên đã 429.
        ...
        # Chặn trên ở capacity: im lặng một ngày cũng không tích vô hạn
    ...
    def consume(self, client_id: str, now: float | None = None) -> None:
        ...
        # Ghi lại CẢ ts — quên thì lần sau tính phần nạp thêm từ mốc cũ,
        # và xô tự đầy lại vô tội vạ.
        self.client.hset(key, mapping={"tokens": tokens - 1, "ts": now})
        self.client.expire(key, BUCKET_TTL_SECONDS)
